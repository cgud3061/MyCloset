...
function openDetail (id) {
    selectedItem.value = props.items[id-1];
        ...
        <div class="h-100 w-100">
            <div className="grid grid-cols-10">
                ...
                <div className="bg-white col-start-3 col-span-11"> 
                    <div v-for="item in items" class="h-40 w-30 bg-white border shadow-sm rounded-xl dark:bg-gray-800 dark:border-gray-700 dark:shadow-slate-700/[.7]">  <!-- このブロックにitemsテーブルから引き出した洋服情報を並べる -->
                        <img class="w-full h-5/6 rounded-t-xl" src="" alt="Image Description">
                        <div class="p-4 md:p-5">
                            <h3 class="text-lg font-bold text-gray-800 dark:text-white">
                                {{ item.name }}
                            </h3>
                            <p class="mt-1 text-gray-800 dark:text-gray-400">
                                {{ categories[item.categorie_id-1].name }}
                            </p>
                            <p class="mt-1 text-gray-800 dark:text-gray-400">
                                {{ brands[item.brand_id-1].name }}
                            </p>
                            <button @click="openDetail(item.id)">詳細</button>
                        </div>
                    </div>
                    <div v-show="showDetail" class="h-100 w-200 flex justify-center items-center bg-white">  <!-- 洋服詳細を表示するモーダル -->
                        <div class="h-full w-full max-w-7xl mx-auto sm:px-6 lg:px-8">
                            <div class="h-full w-full bg-white overflow-hidden shadow-sm sm:rounded-lg">
                                ...
                                <PrimaryButton @click="closeDetail">閉じる</PrimaryButton>
                            </div>    
                        </div>     
                    </div>
