...
function update () {
    Inertia.put("/mycloset/" + selectedItemId, form);
};

// 洋服削除
function deleteItem () {
    Inertia.delete("/mycloset/" + selectedItemId);
};
            ...
            <div class="mx-auto max-w-2xl py-4 px-4 sm:py-4 sm:px-6 lg:max-w-7xl lg:px-8">
                <div class="relative flex bg-white rounded-md border border-gray-200 w-full h-60">
                    <div class="flex justify-center items-center w-1/5">
                        <img class="aspect-square w-10/12 m-2 rounded-full ring-2 ring-white" src="https://images.unsplash.com/photo-1491528323818-fdd1faba62cc?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=facearea&facepad=2&w=256&h=256&q=80" alt="">
                    </div>
                ...
                <div class="mt-6 grid grid-cols-1 gap-y-10 gap-x-6 sm:grid-cols-2 lg:grid-cols-4 xl:gap-x-8">
                    <!-- 洋服要素 -->
                    <div v-for="item in items" class="group relative">
                        <!-- 写真要素 -->
                        <div class="relative min-h-80 aspect-w-1 aspect-h-1 w-full overflow-hidden rounded-md bg-gray-200 lg:aspect-none lg:h-80">
                            ...
                            <button @click="openDescription(item.id)" class="absolute z-10 h-full w-full object-cover object-center hover:opacity-75"></button>
                        </div>
                        <!-- 商品名など -->
                        ...
                        <div class="relative flex w-full items-center overflow-hidden bg-white px-4 pt-14 pb-8 shadow-2xl sm:px-6 sm:pt-8 md:p-6 lg:p-8">
                            ...
                            <div class="grid w-full grid-cols-1 items-start gap-y-8 gap-x-6 sm:grid-cols-12 lg:gap-x-8">
                                <div class="aspect-w-2 aspect-h-3 overflow-hidden rounded-lg bg-gray-100 sm:col-span-8 lg:col-span-7">
                                    <img :src="items[selectedItemId-1].image_url" alt="Two each of gray, white, and black shirts arranged on table." class="h-full w-full object-cover object-center">
                                </div>
                                <div class="h-full mb-4 sm:col-span-4 lg:col-span-5">
                                    <h2 class="text-lg font-bold text-gray-500 sm:pr-12">{{ items[selectedItemId-1].name }}</h2>
                                    <p class="mr-2 text-base text-gray-500">種別：{{ types[items[selectedItemId-1].type_id-1].name }}</p>
                                    <p class="mr-2 text-base text-gray-500">ブランド名：{{ brands[items[selectedItemId-1].brand_id-1].name }}</p>
                                    <p class="mr-2 text-base text-gray-500">カテゴリー：{{ categories[items[selectedItemId-1].categorie_id-1].name }}</p>
                                    <p class="text-base text-gray-500">価格：{{ items[selectedItemId-1].price }}円</p>
                                    <h3 class="mt-3 mb-1 text-base text-gray-500">説明：</h3>
                                    <text class="mb-2 text-sm text-gray-500">{{ items[selectedItemId-1].description }}</text>
